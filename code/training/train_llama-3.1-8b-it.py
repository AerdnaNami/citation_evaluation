...
class PromptCompletionDataset(Dataset):
    ...
    def _validate_rows(self, path: str) -> None:
        ...
        if bad > 0:
            print(f"WARNING Dropped {bad} invalid/empty examples from {path}. Kept {len(cleaned)}.")
...
def main():
    ...
    eval_path_for_training = dev_path if (eval_split == "dev" and dev_path) else eval_path
    ...
    early_stopping_patience = int(run_cfg.get("early_stopping_patience", 3))
    early_stopping_threshold = float(run_cfg.get("early_stopping_threshold", 0.0))
    ...
    if num_label_tokens == 0:
        raise RuntimeError("SANITY CHECK: num_label_tokens == 0 (all labels masked).")
    ...
    print(f"SANITY CHECK: forward loss={out.loss.item()}")
    model.train()

    callbacks = []
    if eval_ds is not None and early_stopping_patience > 0:
        callbacks.append(
            EarlyStoppingCallback(
