...
def main():
    ...
    # Set this in config for SciTulu:
    ...
    # Model-dependent switches (good for SciTulu)
    ...
    # Tokenizer (SciTulu is Llama-family; pad is often missing)
    ...
    # QLoRA config (only if load_in_4bit=True)
    model_kwargs = dict(
        device_map="auto",
    ...
    # Keep model config aligned with tokenizer (important for generation + padding)
    model.config.pad_token_id = tokenizer.pad_token_id
    model.config.eos_token_id = tokenizer.eos_token_id

    print("Model loaded.")

    # FlashAttention (only if your env supports it)
    if use_flash_attn:
        try:
    ...
    # Only prep for k-bit training if quantized
    if load_in_4bit:
        print("Prepping for kbit training and adding LoRA adapters...")
        model = prepare_model_for_kbit_training(model)
    else:
        print("Adding LoRA adapters (no 4-bit quantization)...")
