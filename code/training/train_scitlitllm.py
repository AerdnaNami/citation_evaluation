...
def evaluate_model_on_eval_file(
    ...
    *,
    ...
    Tokenization is truncated to max_length (NOT tokenizer.model_max_length).
    """
...
def main():
    ...
    # NEW: model-dependent switches
    load_in_4bit = bool(run_cfg.get("load_in_4bit", True))
    target_modules = run_cfg.get(
        "lora_target_modules",
    ...
    print(f"  quantization: load_in_4bit={load_in_4bit}")
    ...
    # NEW: safer special-token handling (helps across different model families)
    ...
    # NEW: keep model config in sync with tokenizer ids
    model.config.pad_token_id = tokenizer.pad_token_id
    model.config.eos_token_id = tokenizer.eos_token_id

    print("Model loaded.")

    if use_flash_attn and hasattr(model.config, "attn_implementation"):
        try:
    ...
    lora_cfg = LoraConfig(
        ...
        bias="none",
        task_type="CAUSAL_LM",
        target_modules=target_modules,  # NEW: configurable per model
    )
    ...
    if eval_path and os.path.exists(eval_path):
        try:
            ...
            eval_metrics = evaluate_model_on_eval_file(
                ...
                gold_key=completion_key,
                use_label=True,
                debug_k=debug_k,
                debug_chars=debug_chars,
                iou_threshold=iou_threshold,
                max_length=max_length,  # uses YOUR training/eval max_length
            )
    ...
    entry = {
        ...
        "quantization": "4bit_nf4" if load_in_4bit else "none",
        "max_length": max_length,
        "per_device_train_batch_size": per_device_train_batch_size,
        "per_device_eval_batch_size": per_device_eval_batch_size,
